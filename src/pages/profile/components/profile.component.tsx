...
export default function ProfileComponent({
  ...
  return (
    <main className={`bg-white-bg pb-6 min-h-screen md:pb-10 ${style || ""}`}>
      ...
      >
        <Link
          className="btn btn-primary text-sm mt-4 ml-auto mr-0 text-center"
          to={`/expert/${user?._id}`}
        >
          {user?.role === Role.EXPERT ? 'Ver mi perfil de experto' : 'Ver mi perfil de clínica'}
        </Link>
      </ProfileHeader>
      ...
      <div className="flex content-sections md:px-8 lg:gap-8">
        ...
        {viewport >= 1024 ? <div className="w-full">{children}</div> : null}
      </div>
    </main>
  );
}
