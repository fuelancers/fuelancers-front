      ...
      <div className="flex gap-5 overflow-x-auto max-w-full">
        {!!data?.length ? (
          data.map((item, index: number) => (
            <div
              className="card min-w-[12rem] h-48 w-48 min-h-[12rem] shadow-into border-[1px] border-text-30 rounded-md relative flex-wrap"
              key={`portfolio-${index}`}
            >
              {isOwner ? (
                <ButtonEdit
              ...
              <img
                src={item.portfolio.image}
                alt={item.portfolio.cite}
